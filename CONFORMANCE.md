...
| **specVersion** | 8.2.10, unpublished |
| **Re-derived at this write** | `git -C ../langsys2 ls-tree c1b16560d0a5191a736e63b7975d79e8625db8d3 docs/sdk-spec.mdx` → `e22dad18…`. The 113 rule ids are read out of that blob by `node _dev_/conformance-summary.mjs`, not counted from this file. |
...
| **Shared vectors** | `vectors/server-message-vectors.json`, vendored byte-exact, blob `c8125549cfee0f5286f79a8cbc194cd30ccd446e`. |
| **Suites** | unit 146 tests in 10 files (`npm test -- --run`, no network) · contract 22 assertions (`_dev_/contract/verify-contract.mjs`) · E2E 61 assertions, live (`_dev_/e2e/verify.mjs`) · SRV concurrency 6 assertions (`_dev_/e2e/srv-concurrency.mjs`) |
...
6. **MIG-1..8, SNAP-2, SNAP-3** — no legacy-key mode or snapshot loader in the core. Nothing for
   the binding to wire until there is.
...
| CAT-2 | delegated | - | core row CAT-2 (implemented, n/a (pure)) · probe `/\blookup\(/` binding 0, core 4 |
...
| HINT-7 | delegated | - | core row HINT-7 (implemented, contract) · probe `/429/` binding 0, core 1 |
...
| SRV-1 | partial | n/a (pure) | src/ssr-measure/served-bytes.test.ts, on served bytes. Under README-SSR's component-body seed, `$t()` serves the request locale, and the base language for a genuine miss in the same render; control: an empty catalog serves base. Concurrency through that seed, `_dev_/e2e/srv-concurrency.mjs` on a freshly started dev server, 100 rounds × 8 renders alternating `it-it`/`de-de`: 0 of 800 wrong-locale; the same harness with the seed before an `await` in `load` (positive control) serves 353 of 800 wrong. Svelte's default server render is one synchronous pass, so the seed's placement decides it. `<Translate>` and `<Phrase>` serve the base language with the translation present, because both construct their handler in `$effect`, which SSR never runs — pinned as GAP rows that go red when that changes |
| SRV-2 | not implemented | - | No request scope exists: the catalog is the core's module-global `sTranslations`, and README-SSR's seed writes it per request, which the MUST forbids by construction. The default renderer keeps it correct per visitor (SRV-1: 0 of 800), and the documented placement is what does it (control: 353 of 800). The `experimental.async` shape — an await, then a read in the same script — is unmeasured here: it needs a compiler flag that would change the renderer for every testbed route |
...
| MIG-1 | not implemented | - | core row MIG-1 (not implemented): the core has no legacy-key mode and `mig-vectors.json` is not authored. The binding would take no part — a legacy key resolves through the same `t()` — and records nothing until the core does |
| MIG-2 | not implemented | - | core row MIG-2 (not implemented): the core has no legacy-key mode and `mig-vectors.json` is not authored. The binding would take no part — a legacy key resolves through the same `t()` — and records nothing until the core does |
| MIG-3 | not implemented | - | core row MIG-3 (not implemented): the core has no legacy-key mode and `mig-vectors.json` is not authored. The binding would take no part — a legacy key resolves through the same `t()` — and records nothing until the core does |
| MIG-4 | not implemented | - | core row MIG-4 (not implemented): the core has no legacy-key mode and `mig-vectors.json` is not authored. The binding would take no part — a legacy key resolves through the same `t()` — and records nothing until the core does |
| MIG-5 | not implemented | - | core row MIG-5 (not implemented): the core has no legacy-key mode and `mig-vectors.json` is not authored. The binding would take no part — a legacy key resolves through the same `t()` — and records nothing until the core does |
| MIG-6 | not implemented | - | core row MIG-6 (not implemented): the core has no legacy-key mode and `mig-vectors.json` is not authored. The binding would take no part — a legacy key resolves through the same `t()` — and records nothing until the core does |
| MIG-7 | not implemented | - | core row MIG-7 (not implemented): the core has no legacy-key mode and `mig-vectors.json` is not authored. The binding would take no part — a legacy key resolves through the same `t()` — and records nothing until the core does |
| MIG-8 | not implemented | - | core row MIG-8 (not implemented): the core has no legacy-key mode and `mig-vectors.json` is not authored. The binding would take no part — a legacy key resolves through the same `t()` — and records nothing until the core does |
...
| SNAP-2 | not implemented | - | core row SNAP-2 (not implemented): no snapshot loader. The binding's half is loading one before the first render, through the same seed as SRV-4; there is nothing to load it with yet |
| SNAP-3 | not implemented | - | core row SNAP-3 (not implemented): no snapshot loader, so nothing reads a snapshot yet |
...
| BIND-5 | implemented | n/a (pure) | `t` is the core's signal (surface.test.ts, `toBe`), and `LangsysApp.t` is the core's current `TFunction`: accessor values pass through the proxy unbound (removing that reds 3). `serverMessage` derives a function from `t` and keeps nothing. Own-rule probe `/memo\|[Cc]ache\|new Map\(\|new WeakMap\(/` binding 0, core 8 |
...
| CACHE-2 | delegated | - | core row CACHE-2 (implemented, contract) · probe `/catalogUnavailable/` binding 0, core 11 |
...
| WIRE-3 | delegated | - | core row WIRE-3 (implemented, contract) · probe `/toLowerCase\|getCanonicalLocales/` binding 0, core 24 · locale.test.ts pins the re-exported `canonicalizeLocale` producing lowercase |
...
npm run dev                                       # 127.0.0.1:5173, freshly started
node --env-file=.env _dev_/e2e/verify.mjs         # 61/61, about 110 s

# SRV concurrency — its own freshly started server; no API, no .env
node _dev_/e2e/srv-concurrency.mjs                # body seed 0/800 wrong-locale; control 353/800
```
...
The HINT-13, GATE-10 and MSG-5 mutations ran against core `86871033`; the rest ran against
earlier core builds of this branch and touch only binding code that has not changed since.
