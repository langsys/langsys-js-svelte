...
| Repo state                 | `git merge-base HEAD origin/main` → `73ead62`, **resolved at HEAD `5a86df2`**. Two earlier revisions of this row were wrong: the first transcribed `a00a32f`; the second resolved `242433e` correctly and then went stale in the same commit that merged main, because a merge-base moves and a recorded one does not. Hence the HEAD stamp — a resolved value is only true of the commit it was resolved at.                                                |
...
| Unit suite                 | **102 tests / 6 files**. Evidence is **mixed and stated per row**: most import the real core, but `stores.test.ts` `vi.mock`s it to drive the signal deterministically. An earlier version of this row claimed "no doubles" for the whole suite — false, and corrected here.                                                                                                                                                                                 |
...
| **BIND-6** — wrap the narrowest surface possible | `implemented` | `t`, `currentlyLoadedLocale`, `sTranslations`, `LangsysAppAPI` and `canonicalizeLocale` are re-exported **by reference**, pinned by `toBe` identity. `LangsysApp` is now a **Proxy over the core singleton** with exactly two overrides (`init`, `setWriteGrant` — both adapt Svelte stores), asserted by set-equality against the exported `OVERRIDDEN_MEMBERS`. Per-member reachability and the module's consumer surface are **generated** from the core prototype and the module object, never listed. Exactly one deliberate exception, `writeEnabled`, with an absence assertion and the identity assertions as its positive control. **Correction — see §6:** an earlier revision of this row read `implemented` on the strength of a hand-enumerated wrapper, and the fleet then reported that shape dropping five core methods. Grade is unchanged, but the _reason_ was wrong in both directions and is restated below. |
...
- **BIND-6: the wrapper was replaced, but not for the reason the fleet gave.** Grade before:
  ...
  move; the justification did, in both directions.
    ...
    Vue and Solid each measured five core methods unreachable through their bindings —
    `applyAuthorization`, `getUserLanguagePreferences`, `parseAcceptLanguageHeader`,
    `findBestLocaleMatch`, `resolveLocale` — and BIND-6 was ruled Proxy fleet-wide. Measuring here
    reproduced it exactly: 33 core members, 20 reached, 13 dropped including those five.

    **All thirteen are declared `private` in the core's `.d.ts`.** TypeScript's `private` is erased
    at runtime, so a prototype walk returns implementation detail beside API with nothing to
    distinguish them. The old wrapper exposed **every public member**; no public surface was ever
    unreachable, and exposing those five would have been the binding publishing surface the core
    withholds — BIND-1's adapt-never-widen line crossed in the direction that looks like diligence.

    The type system caught it before the author did: a testbed probe calling one of the five through
    the binding failed `npm run check`, because `Omit<typeof _LangsysApp, …>` correctly does not
    carry private members. That failure was the finding; the probe was reverted.

    The Proxy still lands, on a forward-looking rationale: a **public** member the core adds later is
    exposed automatically, where the enumerated version would have omitted it silently with nothing
    failing. Real hazard, not yet fired. `_dev_/enumerate-core-surface.mjs` publishes the
    measurement, carries a `--self-test` positive control, and prints a PRIVATE-IN-DTS column so the
    next reader cannot repeat the misreading.
